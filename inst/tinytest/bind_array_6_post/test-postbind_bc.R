  ...
  for(iDataX in seq_along(datagens)) {
    for(iDataY in seq_along(datagens)) {
      for(iDataZ in seq_along(datagens)) {
        ...
        x <- array(x.data, c(1, x.dim[2])) |> rep_dim(x.dim)
        y <- array(y.data, c(1, y.dim[2:3])) |> rep_dim(y.dim)
        ...
        out.lst[[counter]] <- bind_array(input, nDims + 1, ndim2bc = 2L, name_along = FALSE, comnames_from = NULL)
